  ...
  return (
    <div className="flex justify-between bg-background/50 border-b backdrop-blur z-50 w-full fixed top-0">
      <div className="mx-auto flex w-full max-w-7xl justify-between p-4 py-1 text-sm">
        ...
        <button
          className="text-black w-10 h-16 relative focus:outline-none md:hidden"
        ...
        >
          ...
          <div className="block w-8 absolute left-1/2 top-1/2 transform -translate-x-1/2 -translate-y-1/2">
            <span
              aria-hidden="true"
              className={`block absolute h-0.5 w-8 bg-current transform transition duration-500 ease-in-out ${isSideMenuOpen ? "rotate-45" : "-translate-y-1.5"
                }`}
            ></span>
            <span
              aria-hidden="true"
              className={`block absolute h-0.5 w-8 bg-current transform transition duration-500 ease-in-out ${isSideMenuOpen ? "opacity-0" : ""
                }`}
            ></span>
            <span
              aria-hidden="true"
...
function MobileNav({ getnavItems, closeSideMenu, closeMenuRef }) {
  // Attach the ref to the close function
  closeMenuRef.current = closeSideMenu;

  return (
    <div 
    // onClick={() => closeSideMenu()} // Close menu on link click
      ...
      <div className="h-full w-[65%] bg-primary-foreground px-4 py-4 rounded-tl-2xl rounded-bl-2xl">
        ...
        <section className="flex flex-col gap-8 mt-4 items-center">
          <Link href='/CMS'>
            <button
              onClick={() => closeSideMenu()} // Close menu on link click
              className="w-full max-w-[150px] font-bold rounded-xl bg-black text-primary px-4 py-2 transition-all hover:text-black hover:bg-primary">
              CMS
            </button>
          </Link>
          ...
          <Link href='/admissionPortal'>
            <button
              onClick={() => closeSideMenu()} // Close menu on link click
              className="w-full max-w-[200px] font-bold rounded-xl bg-black text-primary px-4 py-2 transition-all hover:text-black hover:bg-primary">
              Admission
            </button>
          </Link>
        </section>
