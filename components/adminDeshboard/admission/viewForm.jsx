...
const ViewForm = ({ cnic }) => {
    const [existState, setExistState] = useState({
        personalData: false,
        guardiansData: false,
        degreeProgramInformation: false,
        academicData: false,
        finalStepUploadDocuments: false
    })
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                await getAdmissionState(cnic)
                await getPersonalData(cnic);
                await getGuardiansData(cnic);
                await getDegreeProgramInformation(cnic);
                await getAcademicData(cnic);
                await getUploadDocuments(cnic);
            } catch (error) {
    ...
    const getAdmissionState = async (fcnic) => {
        try {
            const response = await fetch(`/api/admission/admissionstate/${fcnic}`);
            if (!response.ok) {
                throw new Error("Failed to fetch departments");
            }
            const data = await response.json();

            setExistState((prevData) => ({
                ...prevData, // Keep the existing state
                ...data.result  // Add the new personalData
            }));
        } catch (error) {
            alert(error.message);
        }
    ...
    const getGuardiansData = async (fcnic) => {
        try {
    ...
    const getDegreeProgramInformation = async (fcnic) => {
        try {
    ...
    const getAcademicData = async (fcnic) => {
        try {
    ...
    const getUploadDocuments = async (fcnic) => {
        try {
            ...
            console.log(uploadDocumentsExistResult.data)
            setImageUrl(uploadDocumentsExistResult.data)
    ...
    return (
        <div className='relative overflow-hidden'>
            ...
            <div>
                ...
                {
                    existState.degreeProgramInformation ?
                ...
                {
                    existState.finalStepUploadDocuments ?
                        <div className="flex justify-center self-center z-10 m-2">
                            <div className="backdrop-blur-lg border border-black shadow-lg p-12 bg-primary/50 mx-auto rounded-3xl w-full">
                                <div className="mb-7">
